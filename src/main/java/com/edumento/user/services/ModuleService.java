...
public class ModuleService {
  ...
  public ResponseModel getModules() {
    return ResponseModel.done(
        moduleRepository.findAll().stream().map(ModuleListModel::new).toList());
  }
                    ...
                    switch (user.getType()) {
                      case SUPER_ADMIN, SYSTEM_ADMIN ->
                          permissionRepository
                              .findByTypeInAndDeletedFalse(Arrays.asList(UserType.USER))
                              .stream()
                              .collect(
