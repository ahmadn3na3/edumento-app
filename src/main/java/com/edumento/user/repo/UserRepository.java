...
public interface UserRepository extends CrudRepository<User, Long>, JpaSpecificationExecutor<User> {
	...
	Optional<User> findOneByEmailAndDeletedFalseAndOrganizationAndFoundation(String email,
			Organization organization, Foundation foundation);
	...
	Stream<User> findByIdInAndOrganizationAndDeletedFalse(List<Long> users,
	...
	Stream<User> findByGroupsIdInAndOrganizationAndDeletedFalse(Iterable<Long> ids,
			Organization organization);

	Stream<User> findByRolesIdInAndOrganizationAndDeletedFalse(Iterable<Long> ids,
			Organization organization);

	Integer countByRolesIdInAndDeletedFalse(Iterable<Long> ids);

	Stream<User> findByIdInAndOrganizationAndTypeAndDeletedFalse(Iterable<Long> id,
	...
	// @Query(value = "select results.rank from " + "(select u.id As user_id, u.total_score, "
	// + "CASE " + "WHEN @prev_value = u.total_score THEN @curRank "
	// + "WHEN @prev_value\\:=u.total_score THEN @curRank\\:=@curRank + 1 "
	// + "ELSE @curRank\\:=@curRank + 1 " + "END AS rank "
	...
	// @Query(value = "select u.full_name, u.id, "
	// + "CASE "
	// + "WHEN @prev_value = u.total_score THEN @curRank "
	// + "WHEN @prev_value\\:=u.total_score THEN @curRank\\:=@curRank + 1 "
	// + "ELSE @curRank\\:=@curRank + 1 "
	// + "END as rank, u.school, "
	// + "u.thumbnail, u.total_score "
	// + "from user u, (SELECT @curRank\\:=0) r, (SELECT @prev_value\\:=NULL) pv "
