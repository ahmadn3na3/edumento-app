...
public class SpaceNotificationHandler extends AbstractHandler {
  ...
  private void onAcceptJoin(BaseMessage baseMessage) {
    ...
    var user =
        userRepository.findById(spaceJoinMessage.getJoinedInfoMessage().getId()).orElse(null);
    if (user != null) {
                ...
                public void accept(Joined joined) {
                  ...
                  if (joined.getNotification()
                    ...
                    if (enableEditSpaceEmail && joined.getUser().getMailNotification()) {
                      logger.debug(
                          "has notification flag"
                              + joined.getUser().getMailNotification().toString());
                    }
                    mailService.sendNotificationMail(
            ...
            new Consumer<User>() {
              ...
              public void accept(User user) {
                ...
                if (user.getNotification()) {
                  ...
                  if (user.getMailNotification()) {
                    mailService.sendNotificationMail(
                        notificationMessage1, infoMessage, true, false);
                  }
                }
              }
            });
