...
public class DiscussionHadler extends AbstractHandler {
  ...
  protected void onCreate(BaseMessage baseMessage) {
    ...
    if (discussionMessage.getType() == DiscussionType.INQUIRY) {
      utilities
          .getCommunityUserList(discussionMessage.getSpaceId(), discussionMessage.getFrom().getId())
          .stream()
          .filter(
                ...
                public void accept(User user) {
                  ...
                  if (discussionMessage.getBody() != null) {
                    questionBodyLimit =
                        discussionMessage
                            .getBody()
                            .substring(0, Math.min(discussionMessage.getBody().length(), 150));
                  }
                  var notificationMessage =
                      amqNotifier.saveMessage(
                          userInfoMessage,
                          baseNotificationMessage,
                          createMessage(EntityAction.INQUERY_CREATE),
                          null,
