            ...
            new Function<User, ResponseModel>() {
              ...
              public ResponseModel apply(User user) {
                return commentRepository
                    .findOneByIdAndDeletedFalse(id)
                    .map(
                        new Function<Comment, ResponseModel>() {
                          @Override
                                        ...
                                        return joinedRepository
                                            ...
                                            .map(
                                                new Function<Joined, ResponseModel>() {
                                                  @Override
                                                  public ResponseModel apply(Joined joined) {
                                                    if (SpaceRole.VIEWER.equals(
                                                        joined.getSpaceRole())) {
                                                      throw new NotPermittedException();
                                                    }
                                                    return commentService.toggleCommentLike(
                                                        id, user);
                                                  }
                                                })
                                            .orElseThrow(NotFoundException::new);
                                    ...
                                    new Function<Joined, ResponseModel>() {
                                      ...
                                      public ResponseModel apply(Joined joined) {
                                        if (discussion.getOwnerId().equals(user.getId())
                                          ...
                                          joinedRepository.save(joined);
                                          log.debug(
                                              "Discussion with id {} successfully deleted", id);
                                          return ResponseModel.done();
                                        } else {
                                          throw new NotPermittedException();
                                        }
            ...
            new Function<User, PageResponseModel>() {
              ...
              public PageResponseModel apply(User user) {
                return spaceRepository
                    ...
                    .map(
                        new Function<Space, PageResponseModel>() {
                          @Override
                          public PageResponseModel apply(Space space) {
                            ...
                            Page<DiscussionSummaryModel> page =
                                discussionRepository
                                    ...
                                    .map(
                                        new Function<Discussion, DiscussionSummaryModel>() {
                                          @Override
                                          public DiscussionSummaryModel apply(
                                              Discussion discussion) {
                                            DiscussionSummaryModel discussionSummaryModel =
                                                new DiscussionDetailedModel();
                                            mapDiscussionSummary(
                                                discussion, discussionSummaryModel);
