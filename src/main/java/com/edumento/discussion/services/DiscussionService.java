...
public class DiscussionService {
	...
	@Autowired
	public DiscussionService(UserRepository userRepository, DiscussionRepository discussionRepository,
			SpaceRepository spaceRepository, CommentService commentService, JoinedRepository joinedRepository,
		...
		return discussionRepository.findOneByIdAndDeletedFalse(id).map(new Function<Discussion, ResponseModel>() {
			...
			public ResponseModel apply(Discussion discussion) {
				if (discussion.getOwnerId().equals(SecurityUtils.getCurrentUser().getId())) {
					if (discussion.getComments().stream().anyMatch(new Predicate<Comment>() {
						@Override
							...
							public ResponseModel apply(Discussion discussion) {
								...
								.orElseThrow(new Supplier<NotFoundException>() {
									...
									public NotFoundException get() {
										return new NotFoundException("error.discussion.notfound");
									}
								});
					}
				})
						...
						return commentRepository.findOneByIdAndDeletedFalse(id)
								.map(new Function<Comment, ResponseModel>() {
									...
									public ResponseModel apply(Comment comment) {
										return discussionRepository.findOneByIdAndDeletedFalse(comment.getParentId())
	...
	public ResponseModel getAllDiscussions(Long spaceId, DiscussionType type, PageRequest pageRequest) {
	...
	private void mapDiscussionSummary(Discussion discussion, DiscussionSummaryModel discussionSummaryModel) {
		...
		discussionSummaryModel.setCreationDate(DateConverter.convertDateToZonedDateTime(discussion.getCreationDate()));
		userRepository.findById(discussion.getOwnerId()).ifPresent(new Consumer<User>() {
			@Override
