...
public class AssessmentAutoSolvingRunnable implements Runnable {
  ...
  public AssessmentAutoSolvingRunnable(
      long assessmentId,
      long userId,
      long limitDuration,
  ...
  public void run() {
    ...
    if (userAssessment.isPresent()) {
      if (!Arrays.asList(
              AssessmentStatus.FINISHED, AssessmentStatus.NOT_EVALUATED, AssessmentStatus.EVALUATED)
        ...
        .ifPresent(
            new Consumer<UserAssessment>() {
              ...
              public void accept(UserAssessment userAssessment) {
                ...
                userAssessmentModel.setQuestionAnswerModels(
                    questionListMapping(userAssessment.getQuestionAnswerList()));
              }
            });
