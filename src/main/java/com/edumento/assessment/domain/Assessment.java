...
public class Assessment extends AbstractEntity {
	...
	@Column
	private Boolean randomizingQuestion = Boolean.FALSE;
	@Column
	private Boolean viewAnswersAfterSubmit = Boolean.TRUE;
	...
	@JoinTable(name = "challengees", joinColumns = {
			@JoinColumn(name = "assessment_id", nullable = false, updatable = false, foreignKey = @ForeignKey(name = "FK_ASSESSMENT_USER")) }, inverseJoinColumns = {
					@JoinColumn(name = "challengee_id", nullable = false, updatable = false, foreignKey = @ForeignKey(name = "FK_USER_ASSESSMENT")) })
	...
	public int hashCode() {
		...
		return 31 * result + getTitle().hashCode();
	}
}
