...
public interface AssessmentRepository extends AbstractRepository<Assessment, Long> {
	...
	Optional<Assessment> findOneByIdAndDeletedFalseAndOwnerAndAssessmentTypeIn(Long id, User user,
	...
	Page<Assessment> getUserChallenges(AssessmentType type, Long spaceId, Long userId, AssessmentStatus status, Date currentDate, Pageable page);
	
	/** Created by A.Alsayed on 10/01/19. */
	List<Assessment> findByDeletedFalseAndAssessmentStatusNotAndAssessmentTypeAndDueDateLessThan(AssessmentStatus status, AssessmentType type, Date currentDate);
	
	/** Created by A.Alsayed on 29/01/19 for practice migration script */
	List<Assessment> findAllByAssessmentTypeAndAssessmentStatusAndDeletedFalse(AssessmentType type, AssessmentStatus status);
