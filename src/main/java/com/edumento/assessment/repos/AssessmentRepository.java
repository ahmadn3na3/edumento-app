...
public interface AssessmentRepository
    extends JpaRepository<Assessment, Long>, JpaSpecificationExecutor<Assessment> {
  ...
  Optional<Assessment> findOneByIdAndDeletedFalseAndOwnerIdAndAssessmentTypeIn(
      Long id, Long user, AssessmentType... assessmentType);
  ...
  @Query(
      "select a from Assessment a where a.assessmentType=?1 and a.space.id = ?2 and a.deleted=false"
          + " and (a.publish=true or a.owner.id=?3 )")
  Page<Assessment> findAllByAssessmentType(
      AssessmentType type, Long spaceId, Long userId, Pageable page);

  @Query(
      "select a from Assessment a where a.assessmentType=?1 and a.space.id = ?2 and a.deleted=false"
          + " and a.publish=true and a.owner.id=?3 ")
  Page<Assessment> findAllByAssessmentTypeandOwnerId(
      AssessmentType type, Long spaceId, Long userId, Pageable page);
  ...
  @Query(
      "select count(s), s.assessmentType from Assessment s where s.creationDate between ?1 and ?2"
          + " and s.deleted=false and s.space.id=?3 group by s.assessmentType")
  List<Object[]> assessmenttStaticsByTypeAndCreationDateBetween(Date from, Date to, Long spaceId);

  @Query(
      "select count(s), s.assessmentType from Assessment s where s.creationDate between ?1 and ?2"
          + " and s.deleted=false and s.owner.id=?3 group by s.assessmentType")
  ...
  Page<Assessment> getUserChallenges(
      AssessmentType type,
      Long spaceId,
      Long userId,
  ...
  List<Assessment> findAllByAssessmentTypeAndAssessmentStatusAndDeletedFalse(
