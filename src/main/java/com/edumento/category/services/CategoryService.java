...
public class CategoryService {
			...
			public ResponseModel apply(User user) {
				if (createCategoryModel.getOrganizationId() == null && createCategoryModel.getFoundationId() == null) {
					if (user.getType() == UserType.SUPER_ADMIN || user.getType() == UserType.SYSTEM_ADMIN) {
						...
						var categoryMessageInfo = new CategoryMessageInfo(category.getId(),
	...
	public ResponseModel getCategories(PageRequest page, Long foundationId, Long organizationId, String filter,
		...
		return userRepository.findOneByUserNameAndDeletedFalse(SecurityUtils.getCurrentUserLogin()).map(new Function<User, PageResponseModel>() {
			...
			public PageResponseModel apply(User user) {
				...
				Specification<Category> deletedFalse = new Specification<Category>() {
					@Override
					@Nullable
					public Predicate toPredicate(Root<Category> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
						return cb.equal(root.get("deleted"), Boolean.FALSE);
					}
				};
				...
				if (foundationId != null) {
					...
					byFoundation = new Specification<Category>() {
						@Override
						@Nullable
						public Predicate toPredicate(Root<Category> root, CriteriaQuery<?> criteriaQuery,
								CriteriaBuilder criteriaBuilder) {
							return criteriaBuilder.equal(root.get("foundation"),
									foundation);
						}
					};
				} else if (!all) {
					byFoundation = new Specification<Category>() {
						...
						public Predicate toPredicate(Root<Category> root, CriteriaQuery<?> criteriaQuery,
								CriteriaBuilder criteriaBuilder) {
							return criteriaBuilder.isNull(root.get("foundation"));
						}
					};
				}
				...
				if (organizationId != null) {
					...
					byOrganization = new Specification<Category>() {
						@Override
						@Nullable
						public Predicate toPredicate(Root<Category> root, CriteriaQuery<?> criteriaQuery,
								CriteriaBuilder criteriaBuilder) {
				...
				if (filter != null) {
					name = new Specification<Category>() {
						@Override
						@Nullable
						public Predicate toPredicate(Root<Category> root, CriteriaQuery<?> criteriaQuery,
								CriteriaBuilder criteriaBuilder) {
				...
				Page<CategoryModel> categoryModels = categoryRepository
						...
						.map(new Function<Category, CategoryModel>() {
							@Override
							public CategoryModel apply(Category category) {
								return getCategoryModel(category, "en");
							}
						});
	...
	public ResponseModel updateCategory(CreateCategoryModel createCategoryModel, Long id) {
		...
		return userRepository.findOneByUserNameAndDeletedFalse(SecurityUtils.getCurrentUserLogin())
				.map(new Function<User, ResponseModel>() {
					@Override
					public ResponseModel apply(User user) {
						return categoryRepository.findById(id).map(new Function<Category, ResponseModel>() {
							@Override
							public ResponseModel apply(Category category) {
						...
						return categoryRepository.findById(id).map(new Function<Category, ResponseModel>() {
							...
							public ResponseModel apply(Category category) {
								...
								return ResponseModel.done(spaceRepository.findByCategoryAndDeletedFalse(category)
										.map(new Function<Space, SimpleModel>() {
											@Override
											public SimpleModel apply(Space space) {
												return new SimpleModel(space.getId(), space.getName());
											}
										}).collect(Collectors.toSet()));
							}
						}).orElseThrow(NotFoundException::new);
	...
	protected void initializeCategories() throws IOException {
		...
		createCategoryModels.forEach(new Consumer<LoadCategoryModel>() {
			@Override
			public void accept(LoadCategoryModel createCategoryModel) {
