...
public final class PackagerUtil {
  ...
  public static void packageFiles(
      final File source, final Collection<File> files, String keyId, String key)
    ...
    if (key != null && keyId != null) {
      ...
      for (final File file : files) {
        if (file != null) {
          packagerBuilder
              .append("input='")
              .append(file.getName())
              .append("',stream=video,init_segment='video/")
              .append(file.getName())
              .append("',segment_template='video/")
              .append(file.getName().replace(".mp4", ""))
              .append("-$Number$.mint' \\")
              .append('\n');
        }
      }
      ...
      packagerBuilder
          .append(",stream=audio,segment_template=")
          .append("audio$Number$.ts,playlist_name=audio-playlist.m3u8' \\")
          .append('\n');
      for (final File file : files) {
        if (file != null) {
          packagerBuilder
              .append("'input=")
              .append(file.getName())
              .append(",stream=video,segment_template=")
              .append(file.getName())
              .append("-$Number$.ts,playlist_name=")
              .append(file.getName())
      ...
      packagerBuilder
          .append("--hls_master_playlist_output=")
          .append(dir)
          .append("/")
          .append("master.m3u8");
    }
    ...
    fileWriter.close();
    var process =
        Runtime.getRuntime().exec("sh " + fileSh.getAbsolutePath(), new String[] {}, new File(dir));
    process.waitFor();
