...
public class MultipartFileSender {
  ...
  public void serveResource() throws IOException {
    ...
    var lastModifiedObj = Files.getLastModifiedTime(filepath);
    ...
    var lastModified =
        LocalDateTime.ofInstant(
                lastModifiedObj.toInstant(), ZoneId.of(ZoneId.systemDefault().getId()))
    ...
    if (range != null) {
      ...
      if (ifRange != null && !ifRange.equals(fileName)) {
        try {
          var ifRangeTime = request.getDateHeader("If-Range"); // Throws IAE if invalid.
          if (ifRangeTime != -1) {
            ranges.add(full);
          }
  ...
  private static class Range {
    ...
    private static void copy(
        InputStream input, OutputStream output, long inputSize, long start, long length)
        throws IOException {
