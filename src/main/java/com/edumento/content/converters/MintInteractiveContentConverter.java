...
public class MintInteractiveContentConverter {
	...
	private void deleteConflictContents(final File directory) throws IOException {
		final var allFiles = (List<File>) FileUtils.listFiles(directory, null, true);
		// video-js.css
	...
	private String getNewVideoHTML(final String videoElementId, final String newVideoPath, final String poster)
		...
		var ret = NEW_VIDEO_TAG.replace("VIDEO_ID", videoElementId);
		ret = ret.replace("VIDEO_SRC", newVideoPath);
		return ret.replace("VIDEO_POSTER",
				poster == null || poster.trim().isEmpty() ? "app/assets/imgs/VIDEO_POSTER.jpg" : poster);
	}
	...
	public Path convertInteractiveContentDirectorySync(Task task)
		...
		final var interactiveDirectory = uploadParentPathFromTask.toFile();
		final var htmlFiles = (List<File>) FileUtils.listFiles(interactiveDirectory,
				new String[] { "html", "htm" }, true);
		// ----------------------
		for (final File file : htmlFiles) {
			log.debug("html file: {}", file);

			final var doc = Jsoup.parse(file, "utf-8");
			final var links = doc.select("source[type='video/mp4']");
			for (final Element element : links) {
				log.debug("element --> ", element);
				final var videoFile = new File(file.getParent() + "/" + element.attr("src"));

				if (!videoFile.exists()) {
					continue;
				}
				var ffprobe = new FFprobe();
				var ffmpegProbeResult = ffprobe.probe(videoFile.toPath().toString());
				if (ffmpegProbeResult.getStreams() == null || ffmpegProbeResult.getStreams().size() != 2) {
					continue;
				}

				final var index = videoFile.toString().lastIndexOf(".");

				final var videoDirectory = new File(videoFile.toString().substring(0, index));
				var newVideoFile = new File(videoDirectory + "/" + videoFile.getName());
				...
				// -------------------------------------
				final var videoElement = element.parent();
				final var videoElementId = videoElement.id();
				final var poster = videoElement.attr("poster");

				final var ind = element.attr("src").lastIndexOf(".");
				final var newVideoPath = element.attr("src").substring(0, ind) + "/" + "master.m3u8";
				// -------------------------------------
			...
			FileUtils.copyDirectory(new File("/data/resources/app/assets"), new File(file.getParent() + "/assets"));
			// -------------------------------------
		}
		// ----------------------
