...
public class DownloadService {
	...
	public ResponseModel startDownload(Long id, HttpServletRequest request) {
		...
		var contentModel = contentService.getContentInformation(id);
	...
	public void getRange(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// get content Information by id throw services
		var downloadId = request.getHeader("did");
		//
		if (downloadId == null) {
		...
		var task = taskRepository.findOneByIdAndType(downloadId, TaskType.DOWNLOAD);
		...
		if (task == null || task.getStatus() == TaskStatus.FINISHED || task.getStatus() == TaskStatus.CANCELED) {
			...
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			response.setContentType(MediaType.APPLICATION_JSON_VALUE);
			new ObjectMapper().writeValue(response.getOutputStream(), ResponseModel.error(Code.NOT_FOUND));
			return;
		}
		MultipartFileSender.fromPath(fileUtil.createFilePathFromTask(task)).with(request).with(response)
				.serveResource();
	...
	public ResponseModel commit(String id, HttpServletRequest request) {
		...
		var task = taskRepository.findOneByIdAndType(id, TaskType.DOWNLOAD);
		if (task == null || task.getStatus() == TaskStatus.FINISHED || task.getStatus() == TaskStatus.CANCELED) {
