...
public class ContentService {
        ...
        .map(
            new Function<User, ResponseModel>() {
              ...
              public ResponseModel apply(User user) {
                var space =
                    spaceRepository
                        .findById(contentCreateModel.getSpaceId())
                ...
                content.setTags(
                    contentCreateModel.getTags() != null
                        ? String.join(",", contentCreateModel.getTags())
                        : "");
  ...
  public ResponseModel getSpaceContents(Long spaceId, PageRequest pageRequest, String shelf) {
    ...
    return userRepository
        .findOneByUserNameAndDeletedFalse(SecurityUtils.getCurrentUserLogin())
        .map(
            new Function<User, PageResponseModel>() {
              @Override
              public PageResponseModel apply(User user) {
                ...
                Specification<Content> statusSpec =
                    new Specification<Content>() {
                      ...
                      public jakarta.persistence.criteria.Predicate toPredicate(
                        ...
                        return root.get("status").in(ContentStatus.READY, ContentStatus.UPLOADED);
                      }
                    };
                Specification<Content> typeSpec =
                    new Specification<Content>() {
                      @Override
                      @Nullable
                      public jakarta.persistence.criteria.Predicate toPredicate(
                          ...
                          CriteriaQuery<?> criteriaQuery,
                          CriteriaBuilder criteriaBuilder) {
                        return criteriaBuilder.notEqual(root.get("type"), ContentType.WORKSHEET);
                      }
                    };
                Specification<Content> spaceSpec =
                    new Specification<Content>() {
                      @Override
                      @Nullable
                      public jakarta.persistence.criteria.Predicate toPredicate(
                          ...
                          CriteriaQuery<?> criteriaQuery,
                          CriteriaBuilder criteriaBuilder) {
                        return criteriaBuilder.equal(root.get("space").get("id"), spaceId);
                      }
                    };
                Specification<Content> notDeleted =
                    new Specification<Content>() {
                      @Override
                      @Nullable
                      public jakarta.persistence.criteria.Predicate toPredicate(
                ...
                Specification<Content> shelfContentSpecification =
                    shelf == null
                        ? null
                        : (root, criteriaQuery, criteriaBuilder) ->
                            criteriaBuilder.equal(root.get("shelfName"), shelf);

                Specification<Content> searchSpec;
                if (user.getType() == UserType.USER) {
                  searchSpec =
                      Specification.where(spaceSpec)
                          .and(statusSpec)
                          .and(typeSpec)
                          .and(notDeleted)
                ...
                return PageResponseModel.done(
                    contentPage.getContent().stream()
                        .map(ContentService.this::getContentModel)
  ...
  public ResponseModel getSpaceContentsUpdates(Long id, DateModel dateModel) {
    ...
    contentUpdateModel
        ...
        .addAll(
            contentRepository
                .findBySpaceIdAndDeletedFalseAndCreationDateAfter(id, date)
                .filter(
    ...
    contentUpdateModel
        ...
        .addAll(
            contentRepository
                .findBySpaceIdAndDeletedFalseAndLastModifiedDateAfter(id, date)
                .filter(
                    new Predicate<Content>() {
                      @Override
                      public boolean test(Content content) {
    ...
    contentUpdateModel
        ...
        .addAll(
            contentRepository
                .findBySpaceIdAndDeletedTrueAndDeletedDateAfter(id, date)
                .map(Content::getId)
                .collect(Collectors.toList()));
            ...
            new Function<User, ResponseModel>() {
              ...
              public ResponseModel apply(User user) {
                return contentRepository
                    .findOneByIdAndDeletedFalse(contentId)
                    .map(
                        new Function<Content, ResponseModel>() {
                          @Override
                        ...
                        new Function<Content, ResponseModel>() {
                          ...
                          public ResponseModel apply(Content content) {
                            return joinedRepository
                                ...
                                .map(
                                    new Function<Joined, ResponseModel>() {
                                      @Override
                                      public ResponseModel apply(Joined joined) {
                                        if (content.getOwner().equals(joined.getUser())
                                          ...
                                          spaceService.updateSpaceModificationDate(
                                              content.getSpace());
                                          return ResponseModel.done(
                                              null,
                                              new ContentInfoMessage(
  ...
  public ResponseModel getShelves(Long id) {
    ...
    return ResponseModel.done(
        contentRepository
            .findDistinctShelfNameBySpaceIdAndDeletedFalse(id)
            .distinct()
  ...
  private ContentModel getContentModel(Content content) {
    ...
    contentModel
        ...
        .addAll(
            Arrays.asList(
                content.getTags() == null ? new String[] {""} : content.getTags().split(",")));

    contentModel.setLastModifiedDate(
        DateConverter.convertDateToZonedDateTime(content.getLastModifiedDate()));
    contentModel.setCreationDate(
        DateConverter.convertDateToZonedDateTime(content.getCreationDate()));
    contentModel.setOwner(
        content.getOwner().getUserName().equalsIgnoreCase(SecurityUtils.getCurrentUserLogin()));
    contentUserRepository
        .findByUserIdAndContentId(SecurityUtils.getCurrentUser().getId(), content.getId())
        .ifPresent(
            new Consumer<ContentUser>() {
              @Override
        ...
        .map(
            new Function<User, ResponseModel>() {
              ...
              public ResponseModel apply(User user) {
                if (user.getType() == UserType.USER) {
                  var joinedSpace =
                      joinedRepository.findOneBySpaceIdAndUserIdAndDeletedFalse(
                          spaceId, user.getId());
                        ...
                        new Function<Content, ResponseModel>() {
                          ...
                          public ResponseModel apply(Content content) {
                            ...
                            if (content.getShelfName() != null
                              ...
                              throw new ExistException("name");
                            } else if (content.getShelfName() == null
                                && contentRepository
                                    .findOneByNameAndOwnerAndSpaceAndDeletedFalse(
                                        content.getName(), content.getOwner(), space)
                                    .isPresent()) {
                              log.warn("content {} exist", content.getName());
                              throw new ExistException("name");
                            }
  ...
  public ResponseModel updateContentUserData(Long id, ContentUserData contentUserData) {
    ...
    return contentUserRepository
        .findByUserIdAndContentId(SecurityUtils.getCurrentUser().getId(), id)
        .map(
            new Function<ContentUser, ResponseModel>() {
              @Override
  ...
  public Content getContentInformation(Long id) {
    ...
    return userRepository
        .findOneByUserNameAndDeletedFalse(SecurityUtils.getCurrentUserLogin())
        .map(
            new Function<User, Content>() {
              @Override
              public Content apply(User user) {
                return contentRepository
                    .findById(id)
                    .map(
                        new Function<Content, Content>() {
                          @Override
