...
public class ContentUploadService {
  ...
  @Autowired MintH5PInteractiveContentConverter mintH5PInteractiveContentConverter;

  @Autowired MintImagetoPDfConverter imagetoPDfConverter;
  ...
  public ResponseModel startUpload(InitUploadModel initUploadModel) {
    ...
    if (initUploadModel.getChunkSize() <= 0) {
      ...
      task.getChunkList()
          .addAll(fileUtil.createChunks(initUploadModel.getChunkSize(), task.getContentLength()));
    }
  ...
  public ResponseModel commit(String uploadId, HttpServletRequest request) {
    ...
    try {
      ...
      throw new MintException(
          e.getMessage(), e, Code.UNKNOWN, e.getMessage() + ":" + e.getClass().getName());
    }
