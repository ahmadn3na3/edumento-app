...
public interface ContentRepository
  ...
  @Query(
      "Select count(c) from Content c where c.space = ?1 and c.status in ( 2 , 3 )and c.type <> 5"
          + " and c.deleted=false")
  Integer countBySpace(Space space);

  @Query(
      "Select count(c) from Content c where c.space = ?1 and c.owner = ?2 and c.status in ( 2 , 3"
          + " )and c.type <> 5 and c.deleted=false")
  Integer countBySpaceAndUser(Space space, User user);

  @Query(
      "Select distinct c.shelfName from Content c where c.space.id = ?1 and c.type <> 5 and"
          + " c.deleted=false")
  Stream<String> findDistinctShelfNameBySpaceIdAndDeletedFalse(Long spaceId);

  @Query(
      "select count(s), s.type, sum(s.size) from Content s where s.creationDate between ?1 and ?2"
          + " and s.deleted=false and s.space.id=?3 and s.status in ( 2 , 3 )and s.type <> 5 group"
          + " by s.type")
  List<Object[]> contentStaticsByTypeAndCreationDateBetween(Date from, Date to, Long spaceId);

  @Query(
      "select count(s), s.type from Content s where s.creationDate between ?1 and ?2 and"
          + " s.deleted=false and s.owner.id=?3 and s.status in ( 2 , 3 )and s.type <> 5 group by"
          + " s.type")
