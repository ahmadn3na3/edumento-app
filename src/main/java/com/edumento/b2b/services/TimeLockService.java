...
public class TimeLockService {
	...
	private static boolean checkUserForOrganization(User user, Organization organization) {
		...
		if (user.getType() == UserType.ADMIN || user.getType() == UserType.FOUNDATION_ADMIN) {
            ...
            return user.getType() != UserType.ADMIN || user.getOrganization().equals(organization);
		}
		return true;
	}
	...
	public ResponseModel create(TimeLockCreateModel timeLockCreateModel) {
		return userRepository.findOneByUserNameAndDeletedFalse(SecurityUtils.getCurrentUserLogin()).map(user -> {
			...
			Organization organization = organizationRepository.findById(timeLockCreateModel.getOrganizationId())
			...
			Date from = DateConverter.convertZonedDateTimeToDate(timeLockCreateModel.getFromDate());
			Date to = DateConverter.convertZonedDateTimeToDate(timeLockCreateModel.getToDate());
			...
			lock.setToDate(to);
			timeLockCreateModel.getDayModels().forEach(model -> lock.getDays().put(model.getDay(), String.join(",",
			...
			case SYSTEM_ADMIN:
				if (organization != null) {
					timeLockModels = timeLockRepository.findByOrganizationAndDeletedFalse(organization, page)
							.map(this::mapTimeLock);
				} else {
					timeLockModels = timeLockRepository.findAll(page).map(this::mapTimeLock);
				}
				break;
	...
	public ResponseModel getById(Long id) {
		...
		return userRepository.findOneByUserNameAndDeletedFalse(SecurityUtils.getCurrentUserLogin()).map(user -> {
			TimeLock timeLock = timeLockRepository.findById(id).orElseThrow(NotFoundException::new);
			...
			TimeLockModel timeLockModel = mapTimeLock(timeLock);
			timeLockModel.getTimeLockExceptionModels().addAll(timeLock.getTimeLockExceptions().stream()
					.filter(e -> !e.isDeleted()).map(this::mapTimeLockException).collect(Collectors.toSet()));
	...
	public ResponseModel delete(Long id) {
		...
		return userRepository.findOneByUserNameAndDeletedFalse(SecurityUtils.getCurrentUserLogin()).map(user -> {
			...
			userRepository.saveAll(timeLock.getUsers().stream().map(user1 -> {
	...
	public ResponseModel update(Long id, TimeLockCreateModel timeLockCreateModel) {
		...
		return userRepository.findOneByUserNameAndDeletedFalse(SecurityUtils.getCurrentUserLogin()).map(user -> {
			...
			if ((!new Date(timeLock.getFromDate().getTime()).equals(from)
					...
					&& timeLock.getTimeLockExceptions().stream().filter(e -> !e.isDeleted()).count() > 0) {
			...
			timeLockCreateModel.getDayModels().forEach(model -> timeLock.getDays().put(model.getDay(), String.join(",",
					model.getTimeModels().stream().map(TimeModel::toString).collect(Collectors.toList()))));
	...
	public ResponseModel createException(Long id, TimeLockExceptionCreationModel timeLockExceptionCreationModel) {
		...
		TimeLockException timeLockException = new TimeLockException();
		...
		if (timeLock.getTimeLockExceptions().stream().filter(e -> !e.isDeleted())
				.anyMatch(e -> e.equals(timeLockException))) {
	...
	public ResponseModel updateException(Long id, Long timeLockId,
		...
		TimeLock timeLock = timeLockRepository.findById(timeLockId).orElseThrow(NotFoundException::new);

		TimeLockException timeLockException = timeLockExceptionRepository.findById(id)
				.orElseThrow(NotFoundException::new);
	...
	public ResponseModel deleteException(Long id, Long timeLockId) {
		...
		return timeLockExceptionRepository.findOneByIdAndTimeLockIdAndDeletedFalse(id, timeLockId).map(e -> {
	...
	public ResponseModel duplicate(Long id) {
		...
		ResponseModel responseModel = getById(id);
		TimeLockModel timeLockModel = (TimeLockModel) responseModel.getData();
		Organization organization = organizationRepository.findById(timeLockModel.getOrganizationModel().getId())
				.orElseThrow(NotFoundException::new);
		TimeLock lock = new TimeLock();
		...
		timeLockModel.getDayModels().forEach(model -> lock.getDays().put(model.getDay(), String.join(",",
				model.getTimeModels().stream().map(TimeModel::toString).collect(Collectors.toList()))));
		lock.setOrganization(organization);
	...
	private void extractUsers(TimeLockCreateModel timeLockModel, Organization organization, TimeLock lock) {
		...
		if (!timeLockModel.getRoles().isEmpty()) {
			users = userRepository.findByRolesIdInAndOrganizationAndDeletedFalse(timeLockModel.getRoles(), organization)
					.filter(user -> user.getType() == UserType.USER).map(user -> {
		...
		if (!timeLockModel.getUsers().isEmpty()) {
			...
			if (!users.isEmpty()) {
				lock.setUserIds(String.join(",",
						users.stream().map(user -> user.getId().toString()).collect(Collectors.toSet())));
				lock.getUsers().addAll(users);
			}
		} else {
	...
	private TimeLockModel mapTimeLock(TimeLock timeLock) {
		...
		timeLock.getDays().forEach((weekDay, s) -> timeLockModel.getDayModels().add(new DayModel(weekDay, s)));
		if (timeLock.getGroups() != null) {
			timeLockModel.getGroups().addAll(Arrays.stream(timeLock.getGroups().split(",")).filter(s1 -> !s1.isEmpty())
					.map(Long::new).collect(Collectors.toSet()));
		}
		if (timeLock.getRoles() != null) {
			timeLockModel.getRoles().addAll(Arrays.stream(timeLock.getRoles().split(",")).filter(s1 -> !s1.isEmpty())
					.map(Long::new).collect(Collectors.toSet()));
		}
		if (timeLock.getUserIds() != null) {
			timeLockModel.getUsers().addAll(Arrays.stream(timeLock.getUserIds().split(",")).filter(s1 -> !s1.isEmpty())
					.map(Long::new).collect(Collectors.toSet()));
		}
	...
	private TimeLockExceptionModel mapTimeLockException(TimeLockException timeLockException) {
		...
		TimeLockExceptionModel timeLockExceptionModel = new TimeLockExceptionModel();
