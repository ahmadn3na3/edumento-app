...
public class OrganizationService {
  ...
  public ResponseModel create(OrganizationCreateModel createModel) {
    ...
    if (createModel.getFoundationId() != null) {
      foundation = foundationRepository.findById(createModel.getFoundationId()).orElseThrow(NotFoundException::new);
    }
  ...
  public ResponseModel update(Long id, OrganizationCreateModel updateModel) {
    ...
    Organization organization = organizationRepository.findById(id).orElseThrow(NotFoundException::new);
    if (!updateModel.getName().equals(organization.getName())) {
      Optional<Organization> tempOrganization = organizationRepository
            ...
            user -> {
              switch (user.getType()) {
                ...
                case FOUNDATION_ADMIN:
                  Page<Organization> foundationAdminPage = organizationRepository.findByFoundationIdAndDeletedFalse(
      ...
      return organizationRepository
          ...
          .map(
              organization -> {
                ...
                List<TimeLock> timeLocks = timeLockRepository
  ...
  private OrganizationModel mapOrganizationToModel(Organization organization) {
    OrganizationModel organizationModel = OrganizationMapper.INSTANCE.organizationToOrganizationModel(organization);
