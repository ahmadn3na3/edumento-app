...
public class CloudPackageService {
	...
	@Autowired
	public CloudPackageService(CloudPackageRepository cloudPackageRepository, UserRepository userRepository,
	...
	public ResponseModel create(CloudPackageCreateModel cloudPackageCreateModel) {
		...
		if (cloudPackageCreateModel.getPackageType() != PackageType.CUSTOM) {
			cloudPackage = cloudPackageRepository.findByPackageTypeAndNameAndDeletedFalse(
					cloudPackageCreateModel.getPackageType(), cloudPackageCreateModel.getPackageType().name());
		} else {
			cloudPackage = cloudPackageRepository.findByPackageTypeAndNameAndDeletedFalse(
	...
	public ResponseModel update(Long id, CloudPackageCreateModel cloudPackageCreateModel) {
		var cloudPackage = cloudPackageRepository.findById(id).orElseThrow(NotFoundException::new);
	...
	public ResponseModel get(PageRequest pageRequest) {
		Page<CloudPackageModel> cloudPackageModelPage = cloudPackageRepository.findAll(pageRequest)
				.map(new Function<CloudPackage, CloudPackageModel>() {
					@Override
					public CloudPackageModel apply(CloudPackage cloudPackage) {
						var cloudPackageModel = new CloudPackageModel(cloudPackage);
						cloudPackageModel.setNumberOfUsers(userRepository.countByCloudPackageAndDeletedFalse(cloudPackage));
						return cloudPackageModel;
					}
				});
		return PageResponseModel.done(cloudPackageModelPage.getContent(), cloudPackageModelPage.getTotalPages(),
				cloudPackageModelPage.getNumber(), Long.valueOf(cloudPackageModelPage.getTotalElements()).intValue());
	}
	...
	protected void getPermsionForPackage(CloudPackage finalCloudPackage) {
		permissionRepository.findByTypeInAndDeletedFalse(Collections.singleton(UserType.USER)).forEach(new Consumer<Permission>() {
			@Override
			public void accept(Permission permission) {
				var aByte = finalCloudPackage.getPermission().getOrDefault(permission.getKeyCode(), (byte) 0);
				if (aByte.equals((byte) 0)) {
					finalCloudPackage.getPermission().put(permission.getKeyCode(), permission.getCode().byteValue());
				} else {
					finalCloudPackage.getPermission().put(permission.getKeyCode(),
							(byte) (permission.getCode().byteValue() + aByte));
				}
			}
		});
	}
