...
public interface AbstractRepository<T extends AbstractEntity, ID extends Serializable>
	...
	// @Override
	// @Query("update #{#entityName} e set e.deleted=true , e.deletedDate =
	// UTC_TIMESTAMP, e.deletedBy = ?#{principal.username} where e.id=?1")
	// @Modifying
	// void deleteById(ID id);

	// @Override
	// @Query("update #{#entityName} e set e.deleted=true , e.deletedDate =
	// UTC_TIMESTAMP, e.deletedBy = ?#{principal.username} where e =?1")
	// @Modifying
	// void delete(T t);

	/**
	 * changed by A.Alsayed 5-2-2019 Purpose is to allow schedule tasks to delete
	 * entities.
	 */
