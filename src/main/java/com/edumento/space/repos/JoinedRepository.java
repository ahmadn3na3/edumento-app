...
public interface JoinedRepository extends JpaRepository<Joined, Long> {
	...
	Optional<Joined> findOneByUserIdAndSpaceIdAndFavoriteTrueAndDeletedFalse(Long userId, Long spaceId);
	...
	Stream<Joined> findByUserIdAndDeletedTrueAndDeletedDateAfterAndSpaceDeletedFalse(Long userId, Date time);

	Stream<Joined> findByUserIdAndDeletedTrueAndSpaceDeletedTrueAndSpaceDeletedDateGreaterThanEqual(Long userId,
			Date time);

	@Query("SELECT avg(j.rating + 0.0 ) as average FROM Joined j where j.space.id  = :spaceId and j.deleted = false")
	Double getAvarageRatingOnSpace(@Param("spaceId") Long spaceId);

	@Query("update Joined j set  j.lastAccessed= current_timestamp(),j.lastModifiedDate=current_timestamp() where j.space = ?1 and j.user=?2")
	@Modifying
	...
	Optional<Joined> findOneBySpaceIdAndUserIdAndSpaceRoleInAndDeletedFalse(Long spaceId, Long userId,
			SpaceRole... roles);

	@Query("select j from Joined j where j.space.id= ?1 and ?2 in (select userName from User u where j.user.id=u.id)  AND j.deleted = false")
	...
	@Query("Select count(distinct j.space.id) from Joined j where j.groupName = ?1 and j.deleted = false ")
	...
	@Query(value = "select count(*) from joined  where deleted=false and space_id=?1", nativeQuery = true)
