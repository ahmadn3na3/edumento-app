...
public interface JoinedRepository extends JpaRepository<Joined, Long> {
  ...
  @Query(
      "SELECT avg(j.rating + 0.0 ) as average FROM Joined j where j.space.id  = :spaceId and"
          + " j.deleted = false")
  Double getAvarageRatingOnSpace(@Param("spaceId") Long spaceId);

  @Query(
      "update Joined j set  j.lastAccessed="
          + " current_timestamp(),j.lastModifiedDate=current_timestamp() where j.space = ?1 and"
          + " j.user=?2")
  @Modifying
  ...
  @Query(
      "select j from Joined j where j.space.id= ?1 and ?2 in (select userName from User u where"
          + " j.user.id=u.id)  AND j.deleted = false")
